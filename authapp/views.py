...
from authapp.forms import (
    ShopUserLoginForm, ShopUserRegisterForm, ShopUserEditForm
)
    ...
    if request.method == 'POST':
        ...
        if register_form.is_valid():
            ...
            if send_verify_mail(user):
                print('отправлено')
                return render(request, 'authapp/verification_status.html')
        else:
...
def send_verify_mail(user):
    ...
    message = (f"Для подтвердение учетной записи {user.username}"
               f"на портале {settings.DOMAIN_NAME}"
               f"перейдите по ссылке: {settings.DOMAIN_NAME}{verify_link}")

    print(f'form: {settings.EMAIL_HOST_USER}, to {user.email}')
